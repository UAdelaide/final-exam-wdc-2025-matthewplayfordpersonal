...
router.get('/api/summary', async function(req, res, next) {
  try {
    const [rows] = await db.execute(`SELECT Users.username as walker_username, Count(WalkRatings.rating_id), AVG(WalkRatings.rating), Count(WalkApplications.application_id) JOIN WalkRatings ON `)
  } catch (error) {
    console.log(error);
    res.json({ error: 'an error occurred' });
  }

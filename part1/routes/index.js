...
router.get('/api/summary', function(req, res, next) {
  try {
    const [rows] = await db.execute(`SELECT Users.username as walker_username, Count(WalkApplications.application_id), AVG(WalkRatings.rating), Count(WalkApplications.)`)
  } catch (error) {
    console.log(error);
    res.json({ error: 'an error occurred' });
  }
